...
xdescribe('# CommentList Component', () => {
  let log;
  ...
  describe('## With $compile', () => {
    ...
    beforeEach($inject(($log, $compile, $rootScope) => {
      log = $log;
      scope = $rootScope.$new();
      element = angular.element('<tsfn-comment-list my-attribute="{{attr}}" my-one-way-binding="outside"></tsfn-comment-list>');
    ...
    it('should log registration', () => {
      let loaded = ['ngComponent', 'tsfnCommentList', 'loaded'].join(' ');
    ...
    describe('### Controller', () => {
      ...
      beforeEach(() => {
        controller = element.controller('tsfnCommentList');
      });
  ...
  describe('## With $componentController', () => {
    ...
    beforeEach($inject(($rootScope, $componentController) => {
      scope = $rootScope.$new();
      controller = $componentController('tsfnCommentList', { $scope: scope }, {
        myAttribute: 'example',
        myOneWayBinding: '1.5'
      });
    }));
