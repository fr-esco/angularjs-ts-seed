...
This module can be configured as follow
```
...
```
 ...
 The json file, contains localized messages as below:
 ```
 {
...
<div layout-gt-md="row">
  <tsfn-panel-widget flex title="{{ 'SITE_VISITORS' | translate }}"></tsfn-panel-widget>
  <tsfn-panel-widget flex title="{{ 'WARNINGS' | translate }}"></tsfn-panel-widget>
  <tsfn-panel-widget flex title="{{ ::'MEMORY_LOAD' | translate }}"></tsfn-panel-widget>
</div>
```

As you can notice, you can use the angular notation `::`. When it is used, angualr will not attach a watch to the expression, so if the locale is changed, the message will not change until the page is refreshed.
