...
function generator() {
  ...
  var argv = yargs.reset()
    .usage('Usage: gulp gen:provider -n [string] -p [string] -m [string]')
    .alias('n', 'name')
    .string('n')
    .describe('n', 'Provider name')
    .alias('m', 'module')
    .string('m')
  ...
  return gulp.src(PATH.src.blankTemplates.provider)
    .pipe(template({
      name: name,
    ...
    .pipe(notify({
      message: 'Provider files generated in <%= options.folder %>.',
      templateOptions: {
        folder: destPath
      },
      onLast: true
    })).pipe(notify({
      message: 'Remember to register the new ngProvider in <%= options.collector %>.',
      templateOptions: {
...
generator.description = 'Generate Provider template';
...
gulp.task('gen:provider', generator);
